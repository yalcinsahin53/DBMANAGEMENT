...
def _parse_typenames_from_capabilities(xml_bytes: bytes) -> List[str]:
    ...
    # WFS 1.x klasik
    names = [n.text for n in root.findall(
...
def wfs_get_typenames(url: str, username: str = "", password: str = "", timeout: int = 20) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if resp.status_code != 200 or not resp.content:
            return {"ok": False, "data": [], "message": f"GetCapabilities başarısız (HTTP {resp.status_code}).", "debug": debug}

        try:
            typenames = _parse_typenames_from_capabilities(resp.content)
        except Exception as e:
            return {"ok": False, "data": [], "message": f"Capabilities XML parse edilemedi: {e}", "debug": debug}

        if not typenames:
            return {"ok": False, "data": [], "message": "TypeName bulunamadı (servis auth istiyor olabilir veya farklı içerik döndürüyor olabilir).", "debug": debug}

        return {"ok": True, "data": typenames, "message": "", "debug": debug}

    except requests.exceptions.Timeout:
        return {"ok": False, "data": [], "message": "GetCapabilities zaman aşımı.", "debug": "TIMEOUT"}
    except Exception as e:
        return {"ok": False, "data": [], "message": "GetCapabilities çağrısında hata.", "debug": str(e)}
...
def fetch_typenames_for_connection(conn_name: str) -> Dict[str, Any]:
    ...
    if not cres.get("ok"):
        return {"ok": False, "data": [], "message": cres.get("message", ""), "debug": ""}
    ...
    if not url:
        return {"ok": False, "data": [], "message": "Seçilen bağlantıda UrlAdress boş.", "debug": ""}
