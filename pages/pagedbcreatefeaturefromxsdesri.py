...
from core.active_db_scope import (
    get_active_db_scope,
    has_active_db_scope,
    set_active_db_scope,
)
...
def build_dbcreatefeaturefromxsdesri_view(page: ft.Page) -> ft.Control:
    ...
    dd_conn = ft.Dropdown(
        label="Esri Geodatabase Baglanti Secimi",
        hint_text="Kayitli Esri baglantilarindan secim yapiniz",
        expand=True,
    ...
    tf_xsd = ft.TextField(
        label="Secilen XSD Dosyasi",
        hint_text=".xsd dosyasi seciniz",
        read_only=True,
        expand=True,
    )

    btn_pick = ft.ElevatedButton("XSD Dosyasi Sec (.xsd)")
    btn_create = ft.ElevatedButton("Veri Setlerini Olustur (Esri DB)", disabled=True)
    ...
    def api_list_schemas(conn_name: str):
        return page.api.esri_db_list_schemas(conn_name)
    ...
    def api_create(
        ...
        return page.api.esri_create_features_from_xsd(
            conn_name, schema_name, xsd_path, manual_epsg
        )

    async def refresh_esri_connections():
        _set_busy(True, "Esri baglantilari yukleniyor...")
        try:
        ...
        debug_lbl.value = f"UYARILAR: {len(esri)} Esri baglantisi yuklendi."
        page.update()
    ...
    async def _load_schemas_for_conn(conn_name: str):
        ...
        if not conn_name:
            debug_lbl.value = "UYARILAR: baglanti secimi temizlendi."
        ...
        if not isinstance(res, dict) or not res.get("ok"):
            ...
            _notify("Hata", f"{msg}\n\n{dbg}".strip())
            debug_lbl.value = f"UYARILAR: sema listesi alinamadi | {conn_name}"
            _safe_update(debug_lbl)
            return
        ...
        debug_lbl.value = f"UYARILAR: {len(schemas)} sema yuklendi | {conn_name}"
        _safe_update(debug_lbl)
    ...
    def on_schema_change(e):
        ...
        _update_create_button_state()
        debug_lbl.value = (
            f"UYARILAR: sema secildi | {schema_name}"
            if schema_name
            else "UYARILAR: sema secimi temizlendi."
        )
        _safe_update(debug_lbl)
    ...
    async def on_pick_click(e):
        try:
            picked = await ft.FilePicker().pick_files(
                dialog_title="XSD dosyasi seciniz (.xsd)",
                allow_multiple=False,
                allowed_extensions=["xsd"],
            )
        except Exception as ex:
            _notify("Hata", f"Dosya secim penceresi acilamadi.\n{ex}")
            return
        ...
        if not xsd_path:
            _notify("Hata", "Secilen dosya yolu okunamadi.")
            return
        ...
        _safe_update(tf_xsd)
        _update_create_button_state()
        debug_lbl.value = f"UYARILAR: xsd secildi | {xsd_path}"
        _safe_update(debug_lbl)
    ...
    async def on_create_click(e):
        ...
        if not conn_name:
            _notify("Uyari", "Lutfen Esri baglantisi seciniz.")
            return
        if not schema_name:
            _notify("Uyari", "Lutfen sema seciniz.")
            return
        if not xsd_path:
            _notify("Uyari", "Lutfen bir XSD dosyasi seciniz.")
            return

        async def _run_create(manual_epsg: int | None = None):
            _set_busy(True, "XSD'den Esri veri setleri olusturuluyor...")
            try:
                return await asyncio.to_thread(
                    api_create, conn_name, schema_name, xsd_path, manual_epsg
                )
            finally:
                _set_busy(False)
        ...
        async def _prompt_epsg_and_retry(base_message: str, base_debug: str):
            ...
            async def _submit_with_epsg(_):
                ...
                _close_dialog(dlg)
                retry_res = await _run_create(epsg)
                if not isinstance(retry_res, dict) or not retry_res.get("ok"):
                    msg = (retry_res or {}).get("message", "Islem basarisiz.")
                    dbg = (retry_res or {}).get("debug", "")
                    _notify("Hata", f"{msg}\n\n{dbg}".strip())
                    debug_lbl.value = "UYARILAR: olusturma basarisiz."
                    _safe_update(debug_lbl)
                    return

                _notify(
                    "Basarili",
                    retry_res.get("message")
                    or "XSD'den Esri veri seti olusturma tamamlandi.",
                )
                debug_lbl.value = "UYARILAR: olusturma basarili."
                _safe_update(debug_lbl)

            def _cancel(_):
                _close_dialog(dlg)
                _notify("Bilgi", "Islem iptal edildi.")
                debug_lbl.value = "UYARILAR: islem iptal edildi."
                _safe_update(debug_lbl)
            ...
            dlg.actions = [
                ...
                ft.ElevatedButton(
                    "Devam Et",
                    on_click=lambda ev: page.run_task(_submit_with_epsg, ev),
                ),
            ]
            dlg.actions_alignment = ft.MainAxisAlignment.END
            _open_dialog(dlg)

        res = await _run_create()

        if not isinstance(res, dict) or not res.get("ok"):
            msg = (res or {}).get("message", "Islem basarisiz.")
            dbg = (res or {}).get("debug", "")
        ...
        _notify("Basarili", res.get("message") or "XSD'den Esri veri seti olusturma tamamlandi.")
        debug_lbl.value = "UYARILAR: olusturma basarili."
        _safe_update(debug_lbl)
    ...
    if use_active_scope:
        ...
        page.run_task(refresh_esri_connections)
