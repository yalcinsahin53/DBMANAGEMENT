...
def build_dbcheckdublicateanddelete_view(page: ft.Page) -> ft.Control:
    ...
    async def _load_schemas_for_conn(conn_name: str):
        ...
        try:
            res = await asyncio.to_thread(
                page.api.dbcheckdublicateanddelete_list_schemas,
                conn_name,
            )
        finally:
            _set_busy(False)
    ...
    async def _load_datasets_for_schema(schema_name: str):
        ...
        try:
            res = await asyncio.to_thread(
                page.api.dbcheckdublicateanddelete_list_datasets,
                conn_name,
                schema_name,
            )
        finally:
    ...
    async def _load_fields_for_dataset(dataset_name: str):
        ...
        try:
            res = await asyncio.to_thread(
                page.api.dbcheckdublicateanddelete_list_fields,
                conn_name,
                schema_name,
                ds_name,
            )
    ...
    async def on_analyze_click(e):
        ...
        try:
            res = await asyncio.to_thread(
                page.api.dbcheckdublicateanddelete_analyze,
                conn_name,
                schema_name,
                dataset_name,
                field_name,
    ...
    async def on_delete_click(row_ref: str):
        ...
        try:
            res = await asyncio.to_thread(
                page.api.dbcheckdublicateanddelete_delete,
                conn_name,
                schema_name,
                dataset_name,
                row_ref,
