...
def build_featuretoesridb_view(page: ft.Page) -> ft.Control:
    ...
    dd_conn = ft.Dropdown(
        label="ESRI DB Bağlantısı Seçimi",
        hint_text="Kayıtlı ESRI Geodatabase bağlantılarından seçim yapınız",
        expand=True,
        visible=not use_active_scope,
    )

    dd_schema = ft.Dropdown(
        label="ESRI DB Şema Seçimi",
    ...
    btn_import = ft.ElevatedButton("ESRI DB'ye Aktar", disabled=True)
    ...
    def _update_import_button_state():
        ...
        btn_import.disabled = not (bool(conn_name) and bool(schema_name) and bool(file_path))
    ...
    def api_import(conn_name: str, schema_name: str, file_path: str, manual_epsg: int | None = None):
        return page.api.esri_db_import_vector(conn_name, schema_name, file_path, manual_epsg)

    async def refresh_esri_connections():
        _set_busy(True, "ESRI DB bağlantıları yükleniyor...")
        try:
        ...
        debug_lbl.value = f"UYARILAR: {len(esri_conns)} ESRI DB bağlantısı yüklendi."
        try:
            page.update()
        except RuntimeError as ex:
            if "must be added to the page first" not in str(ex).lower():
    ...
    async def _watch_schema_selection():
        while True:
            ...
            _update_import_button_state()
            debug_lbl.value = (
                f"UYARILAR: şema seçildi | {cur}" if cur else "UYARILAR: şema seçimi temizlendi."
            )
            _safe_update(debug_lbl)
    ...
    async def on_import_click(e):
        ...
        if not conn_name:
            _notify("Uyarı", "Lütfen ESRI DB bağlantısı seçiniz.")
            return
        if not schema_name:
            _notify("Uyarı", "Lütfen şema seçiniz.")
            return
        if not file_path:
        ...
        async def _run_import(manual_epsg: int | None = None):
            _set_busy(True, "Vektör veri ESRI DB'ye aktarılıyor...")
            try:
