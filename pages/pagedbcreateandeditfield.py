...
def build_dbcreateandeditfield_view(page: ft.Page) -> ft.Control:
    ...
    async def _load_schemas_for_conn(conn_name: str):
        ...
        try:
            res = await asyncio.to_thread(page.api.createandeditfield_list_schemas, conn_name)
        finally:
            _set_busy(False)
    ...
    async def _load_datasets_for_schema(schema_name: str):
        ...
        try:
            res = await asyncio.to_thread(
                page.api.createandeditfield_list_datasets,
                conn_name,
                schema_name,
            )
        finally:
    ...
    async def _load_fields_for_dataset(dataset_name: str):
        ...
        try:
            res = await asyncio.to_thread(
                page.api.createandeditfield_list_fields,
                conn_name,
                schema_name,
                ds_name,
            )
    ...
    async def _on_add_field_click(e):
        ...
        try:
            res = await asyncio.to_thread(
                page.api.createandeditfield_add_field,
                conn_name,
                schema_name,
                dataset_name,
                field_obj,
    ...
    def _confirm_delete_field(field_name: str):
        ...
        async def _delete_now():
            ...
            try:
                res = await asyncio.to_thread(
                    page.api.createandeditfield_delete_field,
                    conn_name,
                    schema_name,
                    dataset_name,
                    field_name,
    ...
    def _open_edit_field_dialog(field_name: str):
        ...
        async def _save_async():
            ...
            try:
                res = await asyncio.to_thread(
                    page.api.createandeditfield_update_field,
                    conn_name,
                    schema_name,
                    dataset_name,
                    current.get("name") or "",
