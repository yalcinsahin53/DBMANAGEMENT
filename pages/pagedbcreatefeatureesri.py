...
def build_dbcreatefeatureesri_view(page: ft.Page) -> ft.Control:
    ...
    def _sync_geometry_fields():
        ...
        has_geom = gtype != "none"
        tf_geometry_column.disabled = not has_geom
        if not has_geom:
            tf_geometry_column.value = "shape"
        tf_srid.disabled = not has_geom
        _safe_update(tf_geometry_column)
                                ...
                                content=ft.Row(
                                    ...
                                    controls=[
                                        ft.IconButton(
                                            ...
                                            tooltip="Guncelle",
                                            on_click=lambda e, idx=i: _open_edit_field_dialog(
                                                idx
                                            ),
                                        ),
                                        ft.IconButton(
                                            ...
                                            tooltip="Sil",
                                            on_click=lambda e, idx=i: _confirm_delete_field(
                                                idx
                                            ),
                                        ),
                                    ],
    ...
    def _confirm_delete_field(idx: int):
        ...
        dlg.content = ft.Text(f"Silmek Istediginizden Eminmisiniz?\nAlan: {field_name}")
    ...
    def _open_edit_field_dialog(idx: int):
        ...
        tf_edit_name = ft.TextField(
            label="Alan Adi", value=cur.get("name") or "", width=220
        )
        tf_edit_alias = ft.TextField(
            label="Gorunen Ad (Alias)", value=cur.get("alias") or "", width=260
        )
        ...
        sw_edit_nullable = ft.Switch(
            label="Null", value=bool(cur.get("nullable", True))
        )
    ...
    async def _refresh_esri_connections():
        ...
        _reset_schema_dropdown()
        _safe_update(dd_conn)
        _set_debug(f"UYARILAR: {len(esri_conns)} Esri Geodatabase baglantisi yuklendi.")
        _update_create_button_state()
    ...
    async def on_create_click(e):
        ...
        if not conn_name:
            _notify("Uyari", "Lutfen Esri Geodatabase baglantisi seciniz.")
            return
        if not schema_name:
        ...
        try:
            res = await asyncio.to_thread(
                page.api.createesrifeature_create_dataset,
                conn_name,
                schema_name,
                dataset_name,
                dataset_alias,
        ...
        if not isinstance(res, dict) or not res.get("ok"):
            msg = (res or {}).get("message", "Esri DB veri seti olusturulamadi.")
            dbg = (res or {}).get("debug", "")
            _notify("Hata", f"{msg}\n\n{dbg}".strip())
        ...
        _notify(
            "Basarili",
            (
                (res.get("message") or "Esri DB veri seti olusturuldu.")
                ...
                + f"\nDepolama: {data.get('storage') or '-'}"
                + (
                    "\nDogrulanan Alan Aliaslari: "
                    + ", ".join(
                        [
                            f"{k} -> {v}"
                            for k, v in (
                                data.get("verified_field_aliases") or {}
                            ).items()
                            if (v or "").strip()
                        ]
                    )
                    if (data.get("verified_field_aliases") or {})
                    else ""
                )
            ),
        )
        _set_debug(
            "UYARILAR: veri seti olusturuldu | "
            f"{data.get('schema','')}.{data.get('dataset','')} | "
            f"alan aliaslari: {(data.get('verified_field_aliases') or {})}"
        )
