...
def build_dbmatchfeatureandloadesri_view(page: ft.Page) -> ft.Control:
    ...
    def _is_esri(v: str) -> bool:
        s = _normalize_dbtype(v).strip()
        return s in (
            "esri geodatabase",
    ...
    async def _ensure_table_columns(table_name: str):
        ...
        res = await asyncio.to_thread(
            page.api.esri_dbmatch_table_columns, conn_name, schema_name, table_name
        )
    ...
    async def _refresh_esri_connections():
        ...
        esri_items = [
            ...
            and _is_esri(str(r.get("DbType") or ""))
        ]
        ...
        _reset_schema_dropdown()
        page.update()
        _set_debug(f"UYARILAR: {len(esri_items)} Esri baglantisi yuklendi.")
    ...
    async def _load_tables(conn_name: str, schema_name: str):
        ...
        try:
            res = await asyncio.to_thread(
                page.api.esri_dbmatch_schema_tables, conn_name, schema_name
            )
        finally:
            _set_busy(False)
    ...
    async def on_load_structure_click(e):
        ...
        try:
            res = await asyncio.to_thread(page.api.esri_dbmatch_vector_structure, file_path)
        finally:
            _set_busy(False)
