...
router = APIRouter(tags=["db-deletefeature-esri"])

_IDENT_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*$")


class DeleteDatasetsEsriReq(BaseModel):
    conn_name: str
    schema_name: str
...
def _validate_esri_conn(conn_name: str):
    ...
    if not info:
        return None, {"ok": False, "message": "Baglanti bulunamadi.", "debug": conn_name}
...
@router.get("/dbdeletefeatureesri-schemas")
def dbdeletefeatureesri_schemas(conn_name: str):
...
@router.get("/dbdeletefeatureesri-datasets")
def dbdeletefeatureesri_datasets(conn_name: str, schema_name: str):
    conn_name = (conn_name or "").strip()
...
@router.post("/dbdeletefeatureesri-delete-datasets")
def dbdeletefeatureesri_delete_datasets(req: DeleteDatasetsEsriReq):
